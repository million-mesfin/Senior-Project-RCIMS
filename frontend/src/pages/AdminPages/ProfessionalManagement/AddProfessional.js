...
const AddProfessional = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.name) formErrors.name = "Name is required";
    if (!formData.phoneNumber) formErrors.phoneNumber = "Phone Number is required";
    else if (!validatePhoneNumber(formData.phoneNumber)) formErrors.phoneNumber = "Phone number must start with 0, followed by 7 or 9, and be 10 digits long.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const professionalData = { ...formData, role: 'professional' };
      await axios.post('http://localhost:5000/api/professionals/add-professional', professionalData);
      alert('Professional created successfully');
      setFormData({
        name: '',
  ...
  return (
    ...
    <>
      <h1>Add New Professional</h1>
      <form sx={{boxShadow:"none"}} onSubmit={handleSubmit} noValidate>
