  ...
  const ListOfPatients = () => {
    ...
    const fetchPatientDetails = async (patientId) => {
      try {
        ...
        if (patientData.caregiver && Object.keys(patientData.caregiver).length > 0) {
    ...
    const handleAddCaregiver = async (caregiverData) => {
      try {
        ...
        setError(`Failed to add caregiver. ${error.response?.data?.message || error.message}`);
      }
    };
...
<div className="header">
          ...
          <Box sx={{width: "100%",display: "flex", gap: '20px', justifyContent: 'space-between'}}>
           <Button type="submit" onClick={()=> handleEditSubmit} variant="contained" color="primary"
            ...
            > Save</Button>
             <Button type="submit" onClick={() => setIsEditing(false)} variant="contained" color="primary"
           className="w-full flex justify-center bg-gradient-to-r from-cyan-300 to-blue-800  text-gray-100 p-4 rounded-full tracking-wide font-semibold shadow-lg cursor-pointer transition ease-in duration-500"
             >Cancel</Button> 
          </Box>
        ...
        <Box  sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
          ...
          <Button  type= "submit" variant="outlined" onClick={() => setIsAddingCaregiver(false)}>
            Cancel
          </Button>
        </Box>
      </form>
                   ...
                   <div class="header">
                    ...
                    <div className="column1">
                      ...
                      <div className="action-buttons">
                        <button className="edit-button" onClick={() => handleEdit(selectedPatient)}>Edit</button>
                        ...
                        {hasCaregiver ? (
                          ...
                          <button className="CareGiver-button" onClick={() => setIsAddingCaregiver(true)}>Add Caregiver</button>
                        )}
                        {/* <button className="CareGiver-button" onClick={handleNavigateToCaregiverDetail}>Caregiver Details</button> */}
                      </div>
                      </div>
          ...
          <TableBody>
            {filteredPatients.map((patient, index) => (
              <TableRow key={patient._id}>
                ...
                <TableCell>
                  <MoreHorizIcon sx={{color: "#0891b2"}} onClick={() => fetchPatientDetails(patient._id)}/>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
